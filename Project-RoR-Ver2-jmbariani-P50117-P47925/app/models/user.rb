...
validates_length_of :password,:on => :update, :within => 5..20,:if => :password_required?
...
validates_format_of :email, :with => /^([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})$/i, :message => "Invalid email" , :on => :create
validates_length_of :name, :on => :create, :within => 4..12 
validates_presence_of :name, :email, :password, :password_confirmation, :salt , :on => :create
...
def greeting
   Notifications.deliver_newgreeting(self.email, self.name)
end
